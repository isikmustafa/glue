    ...
    namespace integrator
    {
        ...
        void SPPM::findHitPoints(const core::Scene& scene, int x, int y, int id)
        {
            ...
            for (int i = 0; i < bound_x; ++i)
            {
                for (int j = 0; j < bound_y; ++j)
                {
                    auto direct_lo = estimateDirect(scene, ray_pool[i][j], x + i, y + j, glm::vec3(1.0f), false, id);
                    m_hitpoint_pool[x + i][y + j].direct_lo += direct_lo;
                }
            }
        }

        glm::vec3 SPPM::estimateDirect(const core::Scene& scene, geometry::Ray& ray, int x, int y, glm::vec3 beta, bool light_explicitly_sampled, int id)
        {
            ...
            auto iteration = scene.object_to_light.find(intersection.object);
            if (iteration != scene.object_to_light.end())
            {
                if (!light_explicitly_sampled)
                {
                    return iteration->second->getLe(ray.get_direction(), intersection.plane.normal, intersection.distance);
                }
                else
                {
            ...
            if (!intersection.bsdf_material->isSpecular(intersection))
            {
                ...
                hitpoint.beta = beta / chosenbsdf_pdf;
            ...
            if (std::isfinite(f_sum) && f_sum > 0.0f)
            {
                ...
                indirect_lo = f * estimateDirect(scene, ray, x, y, beta * f, light_explicitly_sampled, id);
            }
        ...
        void SPPM::tracePhoton(const core::Scene& scene, int id, float one_over_width)
        {
            ...
            for (int iteration = 0; ; ++iteration)
            {
                ...
                {
                    GridCell cell;
                    cell.x = static_cast<int>(std::floor(intersection.plane.point.x * one_over_width));
                    cell.y = static_cast<int>(std::floor(intersection.plane.point.y * one_over_width));
                    cell.z = static_cast<int>(std::floor(intersection.plane.point.z * one_over_width));
